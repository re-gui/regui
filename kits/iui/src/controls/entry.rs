...
decl_function_component!(pub Entry entry(EntryProps) -> Control);

impl Entry {
    pub fn builder(ui: &UI) -> EntryProps {
        EntryProps::new(ui)
    }
}

fn entry(props: &EntryProps, cx: &mut Cx) -> Control {
    let entry = cx.use_state(|| iui::controls::Entry::new(&props.ui));
